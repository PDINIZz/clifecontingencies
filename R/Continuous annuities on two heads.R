...
caxyn <- function(tableslist,x,n,i,m, status ="joint"){
  ...
  if(status =="joint"){
    ...
    ft <- function(t) {
      exp(-d*t)*((tablex[[1]]@lx[x1+t+1]/tablex[[1]]@lx[x1+1])*(tabley[[1]]@lx[y+t+1]/tabley[[1]]@lx[y+1]))
    }
  ...
  if(status=="last"){
    ...
    ft <- function(t) {
      exp(-d*t)*((tablex[[1]]@lx[x1+t]/tablex[[1]]@lx[x1])+(tabley[[1]]@lx[y+t]/tabley[[1]]@lx[y])-((tablex[[1]]@lx[x1+t]/tablex[[1]]@lx[x1])*(tabley[[1]]@lx[y+t]/tabley[[1]]@lx[y])))
  }
    a=integrate(ft,min,max,subdivisions= 10000,stop.on.error = FALSE,abs.tol = TRUE)
    caxyc=a$value
    return(caxyc)
